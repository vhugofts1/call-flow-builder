...
const ROWS_BASE = [
  { p: "#2451", t: "Equipamento PDV", c: "Equipamento", s: "Em andamento", sla: "03:58", risk: false },
  { p: "#2450", t: "Erro login sistema", c: "Sistema/TI", s: "Aberto", sla: "00:42", risk: true },
  { p: "#2447", t: "Reposição embalagem", c: "Estoque", s: "Resolvido", sla: "—", risk: false },
  { p: "#2440", t: "Falha cobrança", c: "Financeiro", s: "Em andamento", sla: "01:15", risk: true },
  { p: "#2438", t: "Treinamento novo func.", c: "Pessoas", s: "Resolvido", sla: "—", risk: false },
];
...
export const Historico = ({ onSelect, onGlobal, chamadoNovo }: HistoricoProps) => {
  const novo = chamadoNovo?.protocolo
    ...
    : [];
  const ROWS = [...novo, ...ROWS_BASE];

  return (
    <div className="space-y-4">
      {chamadoNovo?.cidade && (
        <div className="rounded-2xl border border-accent/30 bg-accent/5 px-4 py-2 text-xs text-muted-foreground">
          🔒 Você está vendo apenas os chamados de <strong className="text-foreground">{chamadoNovo.cidade.split(",")[0].trim()}</strong>.
          Outras cidades têm visões isoladas.
        </div>
      )}
      ...
      <div className="overflow-hidden rounded-2xl border border-border">
        <table className="w-full text-sm">
          ...
          <tbody>
            {ROWS.map((r) => (
              <tr key={r.p} className={`border-t border-border hover:bg-secondary/50 ${(r as any).novo ? "bg-primary/5" : ""}`}>
